...
if(!empty($_GET['wordsearch']) && !empty($_GET['typesearch']))
{
  ...
  else
  {
    ...
    if($typesearch=='1')
    {
        $sql2="SELECT pet.PetID AS petid,pet.Name AS petname,pet.Species as species,pettype.name AS typename FROM pet,pettype where name LIKE '%$wordsearch%' and pet.pettypeid = pettype.pettypeid";
        $sql = "SELECT pet.PetID AS petid,pet.Name AS petname,pet.Species as species,pettype.name AS typename FROM pet,pettype where name LIKE '%$wordsearch%' and pet.pettypeid = pettype.pettypeid limit $pageprocess,5";
    }
    else
    {
      $sql2="SELECT pet.PetID AS petid,pet.Name AS petname,pet.Species as species,pettype.name AS typename FROM pet,pettype where pet.pettypeid = pettype.pettypeid";
        $sql = "SELECT pet.PetID AS petid,pet.Name AS petname,pet.Species as species,pettype.name AS typename FROM pet,pettype where pet.pettypeid = pettype.pettypeid limit $pageprocess,5";
    }
}
else
{   
...
.btmenu {
 margin: 5%;
      ...
      function PetInfo(rec)
            {
                window.open("staffpetinfo.php?petid=" + rec ,"_self");
            }
      </script>
  </head>
  <body>
...
<br><br>
<center>
<h2>ค้นหาสัตว์เลี้ยง</h2>
<form action="staffsearchpet.php" method="get">
<div style="width: 20%;">
    ...
    <tr align="center">
        ...
        $x=mysqli_num_rows($result);
         ...
         for($i=1;$i<=$count2;$i++)
         {
          ...
          {
           ?>
           <a href='staffsearchpet.php?count=<?php echo $count ?>&page=<?php echo $i ?>&wordsearch=<?php echo $_GET['wordsearch'] ?>&typesearch=<?php echo $_GET['typesearch'] ?>'><?php echo $i; ?>&nbsp;</a>
          ...
          {
            ?>
            <a href='staffsearchpet.php?count=<?php echo $count ?>&page=<?php echo $i ?>'><?php echo $i; ?>&nbsp;</a>
            <?php
          }
            $count+=5;
         }
