...
$sql2="SELECT `treatmentid`, `DateTreatment`, treatment.Status AS Status, `VetID`,treatment.PetID AS petid, `TreatmentRoomID`,pet.Name AS petname,user.Name AS vetname,pettype.Name AS pettype FROM treatment,pet,pettype,user WHERE treatment.VetID = user.UserID and treatment.PetID = pet.PetID and pet.PetTypeID = pettype.PetTypeID and treatment.Status = 2 and vetid = '$userid'";
$sql="SELECT `treatmentid`, `DateTreatment`, treatment.Status AS Status, `VetID`,treatment.PetID AS petid, `TreatmentRoomID`,pet.Name AS petname,user.Name AS vetname,pettype.Name AS pettype FROM treatment,pet,pettype,user WHERE treatment.VetID = user.UserID and treatment.PetID = pet.PetID and pet.PetTypeID = pettype.PetTypeID and treatment.Status = 2 and vetid = '$userid' limit $pageprocess,5";
...
.btmenu {
 margin: 5%;
      ...
      {
        window.open("vetpaydrugmanage.php?treatmentid=" + rec ,"_self");
      }
      </script>
  </head>
  <body>
 <nav class="navbar navbar-expand-lg navbar-light bg-light">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
   ...
   <center><div class="head1"><br>ระบบจ่ายยา</div></center>
   <center>
   <div style="width: 80%;">
...
<thead class="thead-dark">
   <tr bgcolor = "#d5f4e6" align="center">
    ...
    <tr align="center">
        ...
        ?>
        <td><button class="btn btn-info" onclick="petsendpaydrug('<?php echo $row['treatmentid']; ?>')">จ่ายยา</button>
        </td>
         <?php
          echo "</tr>";
        }
        ...
        $x=mysqli_num_rows($result);
         ...
         for($i=1;$i<=$count2;$i++)
         {
           ...
           ?>
            <a href='vetpaydrug.php?count=<?php echo $count ?>&page=<?php echo $i ?>'><?php echo $i; ?>&nbsp;</a>
            <?php
