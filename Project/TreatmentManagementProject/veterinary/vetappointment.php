...
if(!empty($_GET['wordsearch']))
{
  ...
  else
  {
    ...
    $wordsearch = $_REQUEST['wordsearch'];
        $sql2="SELECT `appointmentid`, `dateappoint`, `timeappoint`, `reason`, appointment.Status AS status, pet.Name AS petname, user.Name AS vetname,user.Surname AS vetsurname FROM appointment,pet,user WHERE appointment.PetID = pet.PetID and appointment.VetID = user.UserID and dateappoint LIKE '%$wordsearch%'";
        $sql = "SELECT `appointmentid`, `dateappoint`, `timeappoint`, `reason`, appointment.Status AS status, pet.Name AS petname, user.Name AS vetname,user.Surname AS vetsurname FROM appointment,pet,user WHERE appointment.PetID = pet.PetID and appointment.VetID = user.UserID and dateappoint LIKE '%$wordsearch%' limit $pageprocess,5";
...
{  
  ...
  {
  $sql2="SELECT `appointmentid`, `dateappoint`, `timeappoint`, `reason`, appointment.Status AS status, pet.Name AS petname, user.Name AS vetname,user.Surname AS vetsurname FROM appointment,pet,user WHERE appointment.PetID = pet.PetID and appointment.VetID = user.UserID and dateappoint = '$datestr'";
  $sql = "SELECT `appointmentid`, `dateappoint`, `timeappoint`, `reason`, appointment.Status AS status, pet.Name AS petname, user.Name AS vetname,user.Surname AS vetsurname FROM appointment,pet,user WHERE appointment.PetID = pet.PetID and appointment.VetID = user.UserID and dateappoint = '$datestr' limit $pageprocess,5";
}
}
?>
<!doctype html>
...
<br><br>
<center>
<h2 class="head1">ระบบนัดหมาย</h2>
<form action="vetappointment.php" method="get">
<div style="width: 20%;">
    ...
    <tr align="center">
        ...
        $x=mysqli_num_rows($result);
         ...
         for($i=1;$i<=$count2;$i++)
         {
          ...
          {
           ?>
           <a href='vetappointment.php?count=<?php echo $count ?>&page=<?php echo $i ?>&wordsearch=<?php echo $_GET['wordsearch'] ?>'><?php echo $i; ?>&nbsp;</a>
          ...
          {
            ?>
            <a href='vetappointment.php?count=<?php echo $count ?>&page=<?php echo $i ?>'><?php echo $i; ?>&nbsp;</a>
            <?php
          }
            $count+=5;
         }
