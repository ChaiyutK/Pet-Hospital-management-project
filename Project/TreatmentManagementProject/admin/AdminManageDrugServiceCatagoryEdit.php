...
$dcid = $_REQUEST['dcid'];
$sql ="select dcid,name,type from drugservicecatagory where dcid=$dcid";
...
.btmenu {
 ...
 <nav class="navbar navbar-expand-lg navbar-light bg-light">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
   ...
   <center><div class="head1">แก้ไขหมวดยาและบริการ</div></center>
   <center>
   <form action="AdminManageDrugServiceCatagoryEditSave.php" method="post">
    <table width = '50%' border = "0"  bgcolor = "#FFFFFF">
    <input type="hidden" name="dcid" value="<?php echo $dcid ?>">
            <tr><td colspan="4" align="center">
            รหัสหมวดยาและบริการ :&nbsp;
            <input type="text" size='30' name="dcid" maxlength='3' onkeypress="return isNumber(event)" value="<?php echo $record['dcid']; ?>" disabled>
                </td></tr>
            ...
            <input type="text" name="name" size='30' maxlength='30' value="<?php echo $record['name']; ?>" required>
                ...
                <tr><td colspan="2" align="right">
            ประเภท :&nbsp;
            <td colspan="2" align="type">
          
                    <select name='type' required>
                    <option value=''>กรุณาเลือกประเภท</option>
                    <option value='1' <?=$record['type'] == '1' ? ' selected="selected"' : '';?>>ยา</option>
                    <option value='2' <?=$record['type'] == '2' ? ' selected="selected"' : '';?>>บริการ</option>
                    <option value='3' <?=$record['type'] == '3' ? ' selected="selected"' : '';?>>วัคซีน</option>
                    </select>
