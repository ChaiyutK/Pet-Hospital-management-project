...
<br><br>
<h1 align="center" class="head1 ">จัดการห้องตรวจ</h1>
<center>

    <form action="Managetreatmentroom.php" method="get">
    <div style="width: 20%;">
    <select class="browser-default custom-select" name="typesearch" required>
        <option value = ''>----ประเภทสิ่งที่จะค้นหา----</option>
        <option value = 1>ชื่อห้องตรวจ</option>
        </select>
        <table border="0">
        <tr>
       ...
       <tr bgcolor = "#d5f4e6" align="center">
        ...
        <tr align="center">
            ...
            $count++;
              ...
              echo "<td>$count</td>";
              echo "<td>$row[TreatmentRoomID]</td>";
              echo "<td>$row[Name]</td>";
            ?>
            <td>
            <button class="btn btn-warning" onclick="TreatmentroomEdit('<?php echo $row['TreatmentRoomID']; ?>')">แก้ไขข้อมูล</button>
            <button class="btn btn-danger" onclick="TreatmentroomDel('<?php echo $row['TreatmentRoomID']; ?>')">ลบข้อมูล</button>
            </td>
             <?php
              echo "</tr>";
            }
        ...
        $x=mysqli_num_rows($result);
         ...
         for($i=1;$i<=$count2;$i++)
         {
          ...
          {
           ?>
           <a href='Managetreatmentroom.php?count=<?php echo $count ?>&page=<?php echo $i ?>&wordsearch=<?php echo $_GET['wordsearch'] ?>&typesearch=<?php echo $_GET['typesearch'] ?>'><?php echo $i; ?>&nbsp;</a>
          ...
          {
            ?>
            <a href='Managetreatmentroom.php?count=<?php echo $count ?>&page=<?php echo $i ?>'><?php echo $i; ?>&nbsp;</a>
            <?php
          }
            $count+=5;
         }
    ...
    <center><button type="button" class="btn btn-primary" style="margin-top: 10px;" onclick="window.open('ManagetreatmentroomAdd.php','_self');">เพิ่มห้องตรวจ</button></center>
